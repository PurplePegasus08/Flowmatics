...
export const DataStudio: React.FC<DataStudioProps> = ({
  ...
  useEffect(() => {
    setLocalDescription(datasetDescription);
  }, [datasetDescription]);
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-12 h-full bg-surface-50 dark:bg-surface-900 transition-colors">
        <div className="bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-3xl p-20 text-center max-w-2xl w-full shadow-soft animate-slide-up">
          ...
          <div className="mt-8 mb-10 w-full max-w-md mx-auto text-left">
            <label className="block text-xs font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wider mb-2 ml-1">
              Dataset Goal / Context (Optional)
            </label>
            <textarea
              className="w-full bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl p-4 text-sm text-surface-700 dark:text-surface-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all resize-none h-24"
              placeholder="e.g., I want to analyze sales performance by region and identify churn risk..."
          ...
          <label className="inline-flex items-center gap-3 cursor-pointer bg-indigo-600 hover:bg-indigo-700 text-white px-10 py-4 rounded-xl font-bold text-sm transition-all shadow-lg active:scale-95">
            ...
            <input type="file" className="hidden" accept=".csv,.json" onChange={(e) => e.target.files?.[0] && onFileUpload(e.target.files[0], localDescription)} />
          </label>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-surface-900 overflow-hidden relative transition-colors">
      {/* AI Context Banner */}
      {datasetDescription && (
        <div className="bg-indigo-50/50 dark:bg-indigo-500/5 border-b border-indigo-100 dark:border-indigo-500/10 px-6 py-3 flex items-center justify-between animate-fade-in shrink-0 z-40">
          <div className="flex items-center gap-3">
            <div className="bg-indigo-100 dark:bg-indigo-500/20 p-1.5 rounded-lg">
              <BrainCircuit className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest leading-none mb-0.5">Active Analysis Goal</span>
              <p className="text-sm font-medium text-surface-700 dark:text-surface-200 line-clamp-1 italic">
                "{datasetDescription}"
              </p>
            </div>
          </div>
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors bg-white dark:bg-surface-800 px-3 py-1.5 rounded-lg shadow-sm border border-indigo-100 dark:border-indigo-500/20"
          >
      ...
      {showCalcModal && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 animate-fade-in text-left">
      ...
      {/* Goal Refinement Modal */}
      {isEditingDescription && (
        <div className="fixed inset-0 z-[500] bg-surface-950/40 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in text-left">
          <div className="bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-3xl w-full max-w-lg shadow-2xl animate-scale-up overflow-hidden">
            <div className="p-8">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-bold text-surface-900 dark:text-white">Refine Analysis Goal</h3>
                </div>
                <button onClick={() => setIsEditingDescription(false)} className="text-surface-400 hover:text-surface-600 dark:hover:text-surface-200 transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <p className="text-sm text-surface-500 dark:text-surface-400 mb-6 leading-relaxed font-medium">
                Updating your goal helps the AI provide more relevant insights, visualizations, and automated cleaning steps tailored to what you want to achieve.
              </p>
              <textarea
                className="w-full bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-2xl p-5 text-sm text-surface-700 dark:text-surface-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all resize-none h-40 font-medium"
                placeholder="Describe what you want to discover in this data..."
                value={localDescription}
                onChange={(e) => setLocalDescription(e.target.value)}
                autoFocus
              />
              <div className="flex gap-3 mt-8">
                <button
                  ...
                  className="flex-1 px-6 py-3 rounded-xl border border-surface-200 dark:border-surface-700 text-sm font-bold text-surface-600 dark:text-surface-300 hover:bg-surface-50 dark:hover:bg-surface-700 transition-all"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 px-6 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold shadow-lg shadow-indigo-600/20 transition-all font-outfit"
                >
                  Update Context
                </button>
              </div>
