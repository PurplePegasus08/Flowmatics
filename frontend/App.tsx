...
import { processAutoDashboardItems } from './utils/autoDashboard';
...
function AppContent() {
  ...
  const [plotlyHtml, setPlotlyHtml] = useState<string | null>(null);
  ...
  const handleAddToDashboard = useCallback((config: ChartConfig) => {
    ...
    notify('Insight Pinned to Dashboard', 'success');
  ...
  const handleAutoGenerateDashboard = useCallback(async () => {
    if (!sessionId) {
      notify('Please upload a dataset first to generate insights.', 'error');
      return;
    }
    try {
      notify("Analyzing dataset and generating dashboard...", "info");
      const result = await apiClient.generateDashboard(sessionId);
      if (result.charts && result.charts.length > 0) {
        // Use the main content area width for layout (full width minus sidebar ~240px)
        const canvasEl = document.querySelector('[id="dashboard-grid"]') as HTMLElement | null;
        const canvasWidth = canvasEl?.clientWidth ?? (window.innerWidth - 240);
        const newItems = processAutoDashboardItems(result.charts, canvasWidth);
        setDashboardItems(prev => [...prev, ...newItems]);
        notify(`Dashboard generated with ${newItems.length} insights!`, "success");
      } else {
        notify("No charts could be generated for this dataset.", "info");
      }
    } catch (e: any) {
  ...
  const handleGeneratePlotly = useCallback(async () => {
    if (!sessionId) {
      notify('Please upload a dataset first.', 'error');
      return;
    }
    try {
      setPlotlyLoading(true);
      setPlotlyError(null);
      notify('Generating Plotly AI dashboard…', 'info');
      const result = await apiClient.generatePlotlyDashboard(sessionId);
      setPlotlyHtml(result.html);
      notify('Plotly AI dashboard ready!', 'success');
    } catch (e: any) {
      setPlotlyError(e.message || 'Unknown error');
      notify(`Plotly generation failed: ${e.message}`, 'error');
    } finally {
        ...
        <div className="flex-1 overflow-hidden animate-fade-in relative">
          <div className={currentView === AppView.DASHBOARD ? 'h-full' : 'hidden h-full'} aria-hidden={currentView !== AppView.DASHBOARD}>
            <Dashboard
              ...
              onUndo={() => notify("Undo not implemented for dashboard layout yet", "info")}
              plotlyHtml={plotlyHtml}
              plotlyLoading={plotlyLoading}
              plotlyError={plotlyError}
              onGeneratePlotly={handleGeneratePlotly}
            />
          </div>
