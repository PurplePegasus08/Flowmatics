...
class ProcessingService:
    ...
    def cast_type(self, df: pd.DataFrame, column: str, target_type: str) -> pd.DataFrame:
        """
        ...
        try:
            if target_type == 'int':
                df_new[column] = pd.to_numeric(df_new[column], errors='coerce').fillna(0).astype(int)
            ...
            logger.warning(f"Type cast failed for {column} to {target_type}: {e}")
            
        return df_new

    def calculate_column(self, df: pd.DataFrame, new_col_name: str, expression: str) -> pd.DataFrame:
        """
        Create a new column using a numeric expression.
        Assumes expression uses column names directly.
        """
        df_new = df.copy()
        try:
            # Simple eval for basic arithmetic
            # We use eval in a controlled way if possible, or simple replacement
            # For robustness, we check if columns in expression exist
            df_new[new_col_name] = df_new.eval(expression)
        except Exception as e:
